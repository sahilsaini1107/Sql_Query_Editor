const Result = ({ query, headers, rows, csvData }) =>{
    return (
      <div>
        {query ? (
          <section className="text-gray-600 body-font pl-4 w-screen">
            <div className="flex w-full justify-between mt-6 lg:mt-0 w-screen">
              <div className="font-bold text-center py-4 bg-gray-500 text-white rounded-tl-md rounded-tr-lg">
                Output
              </div>
              <CSVLink
                data={csvData}
              ...
              >
                <button className="flex mx-auto text-white bg-indigo-500 border-0 py-2 h-11 px-4 focus:outline-none hover:bg-indigo-600 rounded text-lg justify-center items-center">
                  ...
                  <span className="pl-2 font-semibold">Download CSV</span>
                </button>
              </CSVLink>
            </div>
